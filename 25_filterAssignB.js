...
const employeeTcs = arrayEmployees.filter((employee)=> {
  ...
  console.log(`Average salary of employee from company Wipro: ${salarySum/wiproEmployee.length}`);
  console.log(`---------------- Step 3 ---------------------`);
  const wiproAndIfyEmployee = arrayEmployees.filter((employee)=>{
    return employee.emp_company == "Wipro" || employee.emp_company == "Infy";
  });
  let sumOfSalary = 0;
  for (const employee of wiproAndIfyEmployee) {
    sumOfSalary =  sumOfSalary   + employee.emp_salary;
  };
  console.log(`Average salary of employee from company Wipro & Infy: ${ sumOfSalary /wiproAndIfyEmployee.length}`);